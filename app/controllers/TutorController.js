...
router.get("/browse/tutors", async function (req, res) {
    try {
        ...
        langResult.forEach(row => {
            // Fix Bug 6: filter out empty strings that slip through
            row.languages.split(',').forEach(l => { if (l.trim()) allLangsSet.add(l.trim()); });
        });
...
router.get("/tutor/:id", async function (req, res) {
    try {
        ...
        if (tutor.full_name) {
            ...
            res.status(404).render("Error404", { title: "Tutor Not Found" });
        }
    } catch (err) {
...
router.post("/book/:id", requireLogin, async function (req, res) {
    try {
        ...
        // Fix Bug 3: guard against invalid/NaN times before comparing
        ...
        res.render("BookingSuccess", { title: "Booking Successful", activePage: "search", tutorId: tutor_id });
    } catch (err) {
...
router.post("/booking/cancel", requireLogin, async function (req, res) {
    ...
    try {
        // Fix Bug 7: use Booking.cancel() instead of a duplicated raw query
